...
class UltraFastAlertsWindow(ctk.CTkToplevel):
    ...
    def render_database_product_card(self, product_data, index):
        ...
        history_btn = ctk.CTkButton(
            ...
            font=("Arial", 12, "bold"),
            width=120, height=35,
            command=lambda: self.show_price_history(asin, name)
        )
        history_btn.pack(side="right", padx=(8, 0))

    def show_price_history(self, asin, product_name):
        """عرض تاريخ أسعار المنتج"""
        if not DB_MANAGER:
            return
        ...
        history_window.title(f"Price History - {product_name[:50]}")
        history_window.geometry("600x400")
        
        # عنوان النافذة
        title_label = ctk.CTkLabel(
            history_window,
            text=f"📊 Price History for {product_name[:60]}{'...' if len(product_name) > 60 else ''}",
            font=("Arial", 16, "bold"),
            text_color="#54fac8"
        )
        ...
        # إطار البيانات
        data_frame = ctk.CTkScrollableFrame(history_window)
        data_frame.pack(fill="both", expand=True, padx=20, pady=10)
        
        # عرض التاريخ
        for i, (price, date, time) in enumerate(history):
            row_color = "#2d3748" if i % 2 == 0 else "#1a202c"
            
            entry_frame = ctk.CTkFrame(data_frame, fg_color=row_color)
...
def scraper_func(section, pages, all_pages):
    ...
    def enhanced_alert_callback(item, old_price, new_price, discount_percent, drop_detected=False):
    ...
    try:
        if section == "All Sections":
            for sec in list(CATEGORIES.keys()):
                ...
                log(f"🔍 Processing section: {sec}", "🟢")
                section_url = CATEGORIES[sec]
                
                loop.run_until_complete(scrape_section_enhanced(
                    sec, section_url, 1, pages, None,  # db=None لأننا نستخدم DatabaseManager داخلياً
                    log_fn=lambda m: log(m, "🟢"),
                    progress_fn=lambda pn: update_progress(pn / pages * 100),
                    stop_flag=stop_flag,
                    discount_alert_cb=enhanced_alert_callback,
                    discount_threshold=ALERT_DISCOUNT
                ))
        else:
            section_url = CATEGORIES[section]
            loop.run_until_complete(scrape_section_enhanced(
                section, section_url, 1, pages, None,
                log_fn=lambda m: log(m, "🟢"),
                progress_fn=lambda pn: update_progress(pn / pages * 100),
                stop_flag=stop_flag,
                discount_alert_cb=enhanced_alert_callback,
        ...
        # إرسال تقرير الجلسة
        if telegram_alerts_enabled[0] and session_stats["real_deals"] > 0:
            threading.Thread(
                target=send_summary_report,
                args=(
                    session_stats["total_products"],
                    session_stats["real_deals"], 
                    session_stats["fake_deals"],
                    section
                ),
                daemon=True
            ).start()
        ...
        # إحصائيات نهائية
        elapsed_time = (datetime.now() - session_stats["start_time"]).total_seconds() / 60
        speed = session_stats["total_products"] / elapsed_time if elapsed_time > 0 else 0
        
        log(f"✅ Session completed! Products: {session_stats['total_products']:,} | "
            f"Real deals: {session_stats['real_deals']} | Speed: {speed:.1f}/min")
        
    except Exception as e:
        log(f"❌ Scraping error: {e}", "⚠️")
    finally:
        running[0] = False
        root.title("LAQTA Ultra - Ready")
...
def start_scraping():
    ...
    log(f"🚀 Started scraping {section} - Ultra Fast Mode!", "🔥")

def stop_scraping():
    """إيقاف عملية الكشط"""
    stop_flag["stop"] = True
    running[0] = False
    root.title("LAQTA Ultra - Stopping...")
    log("🛑 Stopping...", "⚡")
...
def export_json_backup():
    """تصدير نسخة احتياطية JSON مضغوطة"""
    if not DB_MANAGER:
        log("❌ Database not initialized", "⚠️")
        return
    ...
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"backup_products_{timestamp}.json.gz"
        
        # تشغيل التصدير في thread منفصل
        def export_thread():
            export_to_json(filename)
            log(f"📦 JSON backup created: {filename}", "✅")
        
        threading.Thread(target=export_thread, daemon=True).start()
        log("🔄 Creating compressed JSON backup...", "💾")
        
    except Exception as e:
        log(f"❌ Backup error: {e}", "⚠️")

# ==== MAIN ROOT ====
root = ctk.CTk()
root.title("LAQTA Ultra - Enhanced Amazon Product Hunter")
...
subtitle_label = ctk.CTkLabel(
    root,
    text="⚡ Ultra Fast Database-Powered Scraper | SQLite Engine",
    font=("Arial", 14, "bold"),
    text_color="#59ff9d"
)
...
backup_btn.grid(row=0, column=4, padx=8, pady=8, sticky="ew")
...
stats_btn.grid(row=0, column=5, padx=8, pady=8, sticky="ew")
...
def startup_sequence():
    """تسلسل بدء التشغيل"""
    initialize_database()
    log("🚀 LAQTA Ultra initialized - SQLite Engine Ready!", "🔥")
    log("✅ Features: Ultra-fast scraping, Database storage, Real-time analysis", "💡")
    log("⚡ Ready to process thousands of products per minute!", "🎯")
