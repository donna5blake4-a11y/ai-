# clean_test.py - اختبار بسيط ونظيف للنظام
...
def print_banner():
    ...
    print("  يحول 1000+ عرض وهمي الى 15 عرض موثوق يوميا")
    print("=" * 60)
...
class SimpleDealFilter:
    ...
    def filter_deals_simple(self, products_list, target_count=15):
        print(f"بدء فلترة {len(products_list)} منتج للحصول على افضل {target_count} عرض...")
        
        basic_filtered = self.basic_filter(products_list)
        print(f"المرحلة 1: تم فلترة {len(basic_filtered)} منتج من الفلترة الاساسية")
        ...
        print(f"المرحلة 3: تم فلترة {len(price_filtered)} منتج من تحليل الاسعار")
