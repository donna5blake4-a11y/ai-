...
class RealPriceSearcher:
    ...
    async def search_noon(self, product_name: str) -> Optional[float]:
        ...
        try:
            ...
            async with self.session.get(url, headers=headers) as response:
                ...
                # البحث عن الأسعار
                price_elements = soup.find_all(text=re.compile(r'\d+\.?\d*\s*(?:جنيه|ج\.م|EGP|LE)'))
                
                for price_text in price_elements[:3]:  # أول 3 أسعار
                    price = self.extract_price(price_text)
                    if price and 100 <= price <= 50000:  # سعر معقول
                        return price
    ...
    async def search_kanbkam(self, product_name: str) -> Optional[float]:
        ...
        try:
            ...
            async with self.session.get(url, headers=headers) as response:
                ...
                for price_text in price_elements[:3]:
                    price = self.extract_price(price_text)
    ...
    async def search_pricena(self, product_name: str) -> Optional[float]:
        ...
        try:
            query = self.clean_product_name(product_name)
            url = f"https://egypt.pricena.com/search?q={query}"
    ...
    async def search_carrefour(self, product_name: str) -> Optional[float]:
        ...
        try:
            query = self.clean_product_name(product_name)
            url = f"https://www.carrefour.eg/search?q={query}"
